...
const bingoSolver = () => {
    for (draw of drawNumbers) {
        for (card of bingoCards) {
            for (row in card.vals) {
                for (col in card.vals[row]) {
                    if (card.vals[row][col].value === draw) {
                        card.vals[row][col].checked = true;
                        if (checkCardWon(card.vals, row, col)) {
                            card.won = true
                            if (bingoCards.every(e => e.won === true)) {
                                return cardScore(card.vals, draw)
                            }
                        }
                    }
                }
